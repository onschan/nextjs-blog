...
3D 객체 답게 Mesh, Light를 이용하여 입체적인 도형을 장면에 추가하는 방법을 알아보았습니다.

다행히도 이 모든 생성자와 그 특징을 알아야만 3D 객체를 화면에 표현할 수 있는 건 아닙니다.

세상에는 이미 멋지게 만들어진 3D 객체들이 있고, 그 객체를 직접 불러와 렌더링 시킬 수 있는 방법이 있습니다.
